...
- **`debt_work.py`'s direct CLI has been fully removed**, along with `debt-runner.ps1` (a thin wrapper whose only purpose was invoking it; native provider dispatch through it was already dropped per `ROADMAP.md`, with no other live dependent found). Both bypassed `sonar_remedy.py`'s safety layer (project auto-detection, `doctor`, queue↔project registration) — two interfaces to one engine, which already caused a real documentation-drift bug — so this cycle's earlier soft deprecation (stderr warning) is now a full removal, not deferred to a later release: this pack has no external users to protect a deprecation window for. Before removal, every capability `debt_work.py` was the *only* entrypoint for was ported to `sonarremedy` first (see `document`/`defer`/`reconcile` below), so nothing is lost. `docs/work-queue.md`, `docs/agent-contract.md`, `docs/agent-rules.md`, `docs/validation.md`, `AGENTS.md` and `CONTRIBUTING.md` now describe `sonarremedy` as the only CLI onto the queue engine. `tests/test_debt_queue_cli.py`'s `QueueCliTests` (subprocess-drove `debt_work.py`'s CLI) and `RunnerTests.test_facade_default_dry_run_across_four_hosts` (subprocess-invoked `debt-runner.ps1`) were removed with them; `RunnerTests` (drives `debt_runner.py` directly, independent of `debt_work.py`) is otherwise unchanged. `debt_work` is no longer listed in `pyproject.toml`'s `py-modules`.

### Added

- **`sonarremedy document`** — regenerates the deterministic audit-trail `progress.json` + `report.json` (every job/entry/attempt, including the `typesafe_hotspot_risk` advisory field), calling the existing, unchanged `debt_queue.Queue.document()`. This was previously reachable only through `debt_work.py document --execute`; `host-agents/sonar-documenter.md`'s "authoritative report generator" claim is now backed by a real `sonarremedy`/MCP entrypoint (`sonar_remedy_document`). `--execute` writes the files; default is dry-run.
- **`sonarremedy defer`/`sonarremedy reconcile`** — the last two `debt_queue.Queue` methods (`defer`, `reconcile`) with no prior `sonarremedy`/MCP entrypoint now have one (`sonar_remedy_defer`/`sonar_remedy_reconcile`), matching `document`'s pattern. `defer --job ID --reason CODE` skips pending/proposed work without touching target files; `reconcile --receipt FILE --effects none|unknown` resolves an expired lease and never auto-retries — the only way to recover a `reconciliation_required` queue, as documented in `docs/agent-rules.md` ("expired leases require explicit reconciliation") and `docs/work-queue.md`'s "Recovery and limits" table. Both default to dry-run; `--execute` performs the mutation.
