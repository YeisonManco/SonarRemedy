...
def relative_name(name):
    if (not isinstance(name, str) or not re.fullmatch(r'[A-Za-z0-9_. /-]{1,500}', name)
            or name.startswith('/') or any(p in ('', '.', '..') or p.endswith((' ', '.')) for p in name.split('/'))):
...
def validate_config(config):
    fields = {'version', 'target', 'branch', 'policy', 'characterization_reason',
              'test_paths', 'expected_red', 'checks', 'allowed_outputs'}
    if (not isinstance(config, dict) or set(config) != fields or type(config['version']) is not int
            or config['version'] != 1 or config['policy'] not in ('red-first', 'characterization')
    ...
    for key in ('test_paths', 'allowed_outputs'):
        values = config[key]
        if (not isinstance(values, list) or len(values) > 2000 or not all(isinstance(v, str) for v in values)
    ...
    for name in config['allowed_outputs']:
        if (not any(p in ('bin', 'obj', 'artifacts') for p in name.split('/')[:-1])
    ...
    if (not isinstance(red, dict) or len(red) > 100 or any(not isinstance(k, str) or not 1 <= len(k) <= 500
            or not isinstance(v, str) or not 8 <= len(v) <= 500
            or not v.startswith(('Assert.', 'AssertionError:', 'Expected')) for k, v in red.items())):
    ...
    if config['policy'] == 'characterization' and (not config['characterization_reason'].strip() or red):
    ...
    for check in checks:
        ...
        if isinstance(check, dict) and check.get('kind') == 'trx':
            keys.add('report')
        if (not isinstance(check, dict) or set(check) != keys or check['kind'] not in ('build', 'trx')
                or not isinstance(check['name'], str) or not q.TOKEN.fullmatch(check['name'])
                or check['name'].casefold() in seen or type(check['timeout_seconds']) is not int
                or not 1 <= check['timeout_seconds'] <= 1800):
            raise q.Blocked('invalid_configured_check')
        seen.add(check['name'].casefold())
        argv = check['argv']
        if (not isinstance(argv, list) or not 1 <= len(argv) <= 64 or any(not isinstance(a, str)
                or len(a) > 8192 or '\0' in a for a in argv) or not Path(argv[0]).is_absolute()
                or Path(argv[0]).suffix.lower() != '.exe' or Path(argv[0]).stem.lower() in ('git', 'ssh', 'sonar-scanner')):
            raise q.Blocked('invalid_check_argv')
        if any(re.search(r'(?i)(sonar\.(token|login)|--password|--token|authorization:)', a) for a in argv):
...
def configure(work, config, *, approved_sha256=None, execute=False):
    root = validate_config(config)
    sha = q.digest(q.encoded(config))
    if not execute:
        return dict(status='dry-run', checks_sha256=sha, policy=config['policy'])
    ...
    with work._open(write=True, identity=True) as (connection, binding):
        ...
        state = dict(config=config, config_sha256=sha, snapshot=snapshot(root))
    ...
    return dict(status='configured', checks_sha256=sha, snapshot_sha256=q.digest(q.encoded(state['snapshot'])))


class TargetBarrier:
    """Cross-queue Windows mutex plus pack-local durable interruption barrier."""
    def __init__(self, root, control_root=None):
    ...
    def quarantine(self, reason):
        q.write_immutable(self.folder / 'quarantine.json', q.encoded(dict(reason=reason)))
...
def _assert_snapshot(root, expected, config, *, generated=False):
    try:
...
def _checks(work, binding, config, folder, phase, expected, runner, deadline, *, red=False):
    def identity():
        try:
    ...
    for check in config['checks']:
        ...
        run.mkdir()
        argv = [a.replace('{run}', str(run)).replace('{target}', binding['root']) for a in check['argv']]
        ...
        receipt = dict(name=check['name'], kind=check['kind'], exit_code=result['exit_code'])
        if check['kind'] == 'trx':
            report = run / check['report']
            receipt['tests'] = parse_trx(report, started, result['exit_code'], config['expected_red'] if red else None)
...
def _replacement_bytes(original, edit):
...
def _apply(root, edits, phase, originals, expected):
...
def integrate(work, job_id, *, execute=False, control_root=None, process_runner=None, deadline=None):
    if not execute:
        return dict(status='dry-run', action='integrate', job_id=job_id)
    ...
    with work._open() as (connection, binding):
        ...
        root = validate_config(config)
        if q.digest(q.read_bytes(work.state / 'execution-config.json', 8*q.MAX_EXPORT)) != configured['initial_artifact_sha256']:
            raise q.Blocked('execution_config_artifact_mismatch')
    with TargetBarrier(root, control_root) as barrier:
        with work._open(write=True, identity=True) as (connection, binding):
            configured = q.parse_json(connection.execute("SELECT value FROM meta WHERE key='executor'").fetchone()[0])
            expected = _assert_snapshot(root, configured['snapshot'], config)
            job = connection.execute('SELECT * FROM jobs WHERE id=?', (work._id(job_id),)).fetchone()
            if not job:
                raise q.Blocked('unknown_job')
            if job['status'] == 'locally_verified':
                return dict(status='already_locally_verified', job_id=job_id)
            if job['status'] != 'proposed':
                raise q.Blocked('only_proposed_jobs_can_integrate')
            attempt = connection.execute('SELECT * FROM attempts WHERE job_id=? ORDER BY number DESC', (job_id,)).fetchone()
            ...
            implementation = [edit for edit in proposal['edits'] if edit.get('phase', 'implementation') == 'implementation']
            if (not implementation or (config['policy'] == 'red-first' and not test_edits)
                ...
                work._set_status(connection, job_id, 'deferred', 'proposal_has_no_approved_behavioral_proof_plan')
                return dict(status='deferred', job_id=job_id, reason='proposal_has_no_approved_behavioral_proof_plan')
            if set(context['write_paths']) & set(config['allowed_outputs']):
                raise q.Blocked('write_set_cannot_be_snapshot_exclusion')
            originals = {edit['path']: q.read_bytes(q.source_path(root, edit['path']), q.MAX_SOURCE) for edit in proposal['edits']}
            ...
            intent = dict(job_id=job_id, attempt_id=attempt['id'], queue=str(work.state),
                          config_sha256=configured['config_sha256'], before=expected,
                          result_sha256=attempt['result_sha'], write_paths=sorted(originals))
            q.write_immutable(integration / 'intent.json', q.encoded(intent))
            barrier.activate(dict(job_id=job_id, queue=str(work.state), intent=str(integration / 'intent.json')))
            phase = 'baseline'
            try:
                expected = _checks(work, binding, config, integration, phase, expected, runner, deadline)
                if config['policy'] == 'red-first':
                    ...
                    expected = _checks(work, binding, config, integration, phase, expected, runner, deadline, red=True)
                phase = 'green'
                ...
                phase = 'post-batch'
                expected = _checks(work, binding, config, integration, phase, expected, runner, deadline)
                proof = dict(status='locally_verified', job_id=job_id, policy=config['policy'],
                             config_sha256=configured['config_sha256'], after=expected,
                             sonar_confirmed=False, notice='Local configured checks only; not authenticated or global quality proof.')
                q.write_immutable(integration / 'verification.json', q.encoded(proof))
                artifacts = {path.relative_to(work.state).as_posix(): q.digest(q.read_bytes(path, q.MAX_SOURCE))
                             for path in integration.rglob('*') if path.is_file()}
                connection.execute('INSERT INTO meta VALUES (?,?)',
                                   ('verification:' + attempt['id'], q.encoded(artifacts).decode()))
                configured['snapshot'] = expected
                connection.execute("UPDATE meta SET value=? WHERE key='executor'", (q.encoded(configured).decode(),))
                work._set_status(connection, job_id, 'locally_verified')
                connection.execute("UPDATE attempts SET status='locally_verified' WHERE id=?", (attempt['id'],))
                outcome = dict(status='locally_verified', job_id=job_id, evidence=str(integration / 'verification.json'))
            except Exception as error:
                reason = str(error) if isinstance(error, q.Blocked) else 'integration_operation_failed'
                q.write_immutable(integration / 'failure.json', q.encoded(dict(phase=phase, reason=reason)))
                if phase == 'baseline' and isinstance(error, q.Blocked) and not isinstance(error, Contaminated):
                    work._set_status(connection, job_id, 'deferred', reason)
                    outcome = dict(status='deferred', job_id=job_id, reason=reason)
                else:
                    ...
                    outcome = dict(status='quarantined', job_id=job_id, reason=reason)
        if outcome['status'] != 'quarantined':
            barrier.finish()  # Clear only after the SQLite transaction committed.
        return outcome


def parse_trx(path, started_ns, exit_code, expected_red=None):
    path = q.local_path(path, exists=True)
    raw = q.read_bytes(path, q.MAX_EXPORT)
    if (not started_ns <= path.stat().st_mtime_ns <= time.time_ns()
            or type(exit_code) is not int or b'<!DOCTYPE' in raw.upper() or b'<!ENTITY' in raw.upper()):
        raise q.Blocked('stale_or_unsafe_trx')
    try:
        root = ET.fromstring(raw)
        nodes = lambda tag: [n for n in root.iter() if n.tag.rsplit('}', 1)[-1] == tag]
        counters, summaries, results = nodes('Counters'), nodes('ResultSummary'), nodes('UnitTestResult')
        if len(counters) != 1 or len(summaries) != 1:
            raise ValueError()
        values = {name: int(counters[0].attrib[name]) for name in ('total', 'executed', 'passed', 'failed', 'notExecuted')}
        if (any(v < 0 for v in values.values()) or values['total'] <= 0 or values['notExecuted'] != 0
                or values['executed'] != values['total'] or values['passed'] + values['failed'] != values['total']
        ...
        for result in results:
            ...
            seen.add(name)
            if outcome == 'Failed':
                messages = [n.text or '' for n in result.iter() if n.tag.rsplit('}', 1)[-1] == 'Message']
                if len(messages) != 1:
                    raise ValueError()
        ...
        if expected_red is not None:
            if (exit_code != 1 or not failures or set(failures) != set(expected_red)
                    or summaries[0].get('outcome') != 'Failed'
                    or any(not failures[name].startswith(marker) for name, marker in expected_red.items())):
                raise ValueError()
        elif exit_code != 0 or failures or summaries[0].get('outcome') not in ('Completed', 'Passed'):
            raise ValueError()
