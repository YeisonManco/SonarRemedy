...
def local_path(value: str | Path, *, exists: bool = False) -> Path:
    ...
    for part in reversed((path, *path.parents)):
        ...
        if part != Path(part.anchor):
            matches = [
                p.name for p in part.parent.iterdir() if p.name.casefold() == part.name.casefold()
            ]
...
def source_path(root: Path, value: str) -> Path:
    if (
        ...
        or not re.fullmatch(r"[A-Za-z0-9_. /-]{1,500}", value)
        or value.startswith("/")
        or any(
            p in ("", ".", "..") or p.endswith((" ", ".")) or hidden_secret(p)
...
def state_path(state: str | Path, root: Path) -> Path:
...
def git_identity(root: str | Path) -> dict[str, str]:
    ...
    return {
        "root": str(local_path(values[0], exists=True)),
        "branch": values[1],
        "revision": values[2],
    }


def check_identity(binding: dict[str, str], reader: Callable[[Path], dict[str, str]]) -> None:
...
def plan(
    ...
    *,
    ...
    if (
        ...
        or not re.fullmatch(r"[A-Za-z0-9_./-]{1,200}", branch)
    ):
    ...
    if (
        ...
        or not TOKEN.fullmatch(data["revision"])
        or not isinstance(data.get("issues"), list)
        or len(data["issues"]) > MAX_ISSUES
    ):
    ...
    binding = {
        ...
        "entry_count": len(data["issues"]),
    }
    counts = Counter(
        i.get("id") for i in data["issues"] if isinstance(i, dict) and isinstance(i.get("id"), str)
    )
    ...
    for ordinal, item in enumerate(data["issues"]):
        ...
        try:
            ...
            for name in paths:
                ...
                if name not in snapshots:
                    ...
                    snapshots[name] = {"sha256": digest(content), "lines": len(text.splitlines())}
        ...
        entry = {
            "ordinal": ordinal,
            "item_sha256": digest(encoded(item)),
            "issue": issue,
            "status": "deferred" if reason else "pending",
            "reason": reason,
        }
    ...
    known_paths = {
        i.get("path") for i in (e["issue"] for e in entries) if isinstance(i.get("path"), str)
    }
    ...
    return {
        "binding": binding,
        "entries": entries,
        "jobs": sorted(groups.values(), key=lambda j: j["id"]),
    }


def write_immutable(path: str | Path, data: bytes) -> None:
    ...
    try:
        ...
        if read_bytes(path, max(MAX_EXPORT, MAX_RESULT)) != data:
            raise Blocked("immutable_artifact_collision") from None
    finally:
        scratch.unlink()
...
def slice_queue(
    ...
    *,
    ...
    try:
        ...
        with connection:
            connection.executemany(
                ...
                [
                    ("version", "1"),
                    ("binding", encoded(preview["binding"]).decode()),
                    ("ready", "false"),
                    ("quarantined", "false"),
                ],
            )
    ...
    return {
        "status": "created",
        "state": str(directory),
        "entries": len(preview["entries"]),
        "jobs": len(preview["jobs"]),
...
class Queue:
    ...
    @contextmanager
    def _open(
        self, *, write: bool = False, identity: bool = False, timeout: int = 10
    ...
    def _integrity(self, connection: sqlite3.Connection) -> int:
        self._scan_tree()
        jobs = connection.execute("SELECT id, data FROM jobs").fetchall()
        if len(jobs) > MAX_ISSUES:
            raise Blocked("job_budget_exceeded")
        binding = parse_json(
            connection.execute("SELECT value FROM meta WHERE key='binding'").fetchone()[0]
        )
        entries = connection.execute(
            "SELECT ordinal, job_id, status FROM entries ORDER BY ordinal"
        ).fetchall()
        expected = sorted(
            (i["ordinal"], row["id"]) for row in jobs for i in parse_json(row["data"])["issues"]
        )
        if (
            len(entries) != binding["entry_count"]
            or [r["ordinal"] for r in entries] != list(range(len(entries)))
        ...
        for row in jobs:
            ...
            known = {
                r[0]
                for r in connection.execute("SELECT id FROM attempts WHERE job_id=?", (row["id"],))
            }
        ...
        for attempt in connection.execute("SELECT * FROM attempts"):
            ...
            if attempt["result_sha"]:
                ...
                gaps += 1
            if (
                attempt["status"] == "locally_verified"
                and not connection.execute(
                    "SELECT 1 FROM meta WHERE key=?", ("verification:" + attempt["id"],)
                ).fetchone()
            ):
        ...
        for row in connection.execute("SELECT value FROM meta WHERE key LIKE 'verification:%'"):
            ...
            for name, sha in artifacts.items():
                ...
                if (
                    not path.is_relative_to(self.state)
                    or digest(read_bytes(path, MAX_SOURCE)) != sha
                ):
        ...
        if (
            native
            and digest(read_bytes(self.state / "native-profile.json", MAX_CONTEXT)) != native[0]
        ):
    ...
    @staticmethod
    def _clock(now: float | int | None) -> float:
    ...
    def _jobs(connection: sqlite3.Connection) -> list[dict[str, Any]]:
        return [
            dict(parse_json(row["data"]), status=row["status"], reason=row["reason"])
            for row in connection.execute("SELECT * FROM jobs ORDER BY id")
        ]

    def _next(self, connection: sqlite3.Connection, limit: int) -> list[dict[str, Any]]:
        if type(limit) is not int or not 1 <= limit <= 8:
            raise Blocked("batch_limit_must_be_1_to_8")
        if (
            connection.execute("SELECT value FROM meta WHERE key='quarantined'").fetchone()[0]
            == "true"
        ):
            return []
    ...
    def next(self, limit: int = 4) -> list[dict[str, Any]]:
    ...
    @staticmethod
    def _set_status(
        connection: sqlite3.Connection, job_id: str, status: str, reason: str = ""
    ) -> None:
        connection.execute(
    ...
    def _source_context(self, binding: dict[str, Any], job: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        for name in job["write_paths"]:
            ...
            windows = [
                {"start_line": start + 1, "end_line": end, "text": "".join(lines[start:end])}
    ...
    def _attempt_folder(self, attempt: sqlite3.Row | dict[str, str]) -> Path:
        return local_path(
            self.state / "jobs" / self._id(attempt["job_id"]) / "attempts" / self._id(attempt["id"])
        )
    ...
    def claim(
        ...
        *,
        ...
        if not execute:
            return {"status": "dry-run", "action": "claim", "next": self.next()}
        now = self._clock(now)
        ...
        with self._open(write=True, identity=True) as (connection, binding):
            ...
            try:
                ...
                return {"status": "deferred", "job_id": job_id, "reason": reason}
            folder = self._attempt_folder({"job_id": job_id, "id": attempt_id})
            ...
            return {
                ...
                "context_path": str(folder / "job.json"),
                "expires_at": now + lease_seconds,
            }

    def _bound_attempt(
        self, connection: sqlite3.Connection, receipt: dict[str, Any]
    ) -> sqlite3.Row:
    ...
    def _validate_proposal(
        ...
        if (
            set(proposal) != fields
            or type(proposal["version"]) is not int
            or proposal["version"] not in (1, 2)
            or proposal["status"] not in ("proposed", "deferred", "failed")
            or not isinstance(proposal["edits"], list)
            or len(proposal["edits"]) > 8
            or not isinstance(proposal["reason"], str)
            or (proposal["reason"] and not TOKEN.fullmatch(proposal["reason"]))
            or not isinstance(proposal["risks"], list)
            or len(proposal["risks"]) > 8
        ...
        for edit in proposal["edits"]:
            ...
            if (
                ...
                or (
                    proposal["version"] == 2 and edit.get("phase") not in ("test", "implementation")
                )
            ):
            ...
            if (
                edit["before_sha256"] != sources[name]["sha256"]
    ...
    def complete(
        self, proposal: dict[str, Any], *, execute: bool = False, now: float | None = None
    ) -> dict[str, Any]:
        if not execute:
            return {"status": "dry-run", "action": "complete"}
        ...
        with self._open(write=True, identity=True) as (connection, binding):
            ...
            context = parse_json(
                read_bytes(self._attempt_folder(attempt) / "job.json", MAX_CONTEXT)
            )
            self._validate_proposal(proposal, context, binding)
            envelope = {
                "version": 1,
                "binding_sha256": digest(encoded(binding)),
                "job_id": attempt["job_id"],
                "attempt_id": attempt["id"],
                "context_sha256": attempt["context_sha"],
                "proposal": proposal,
            }
    ...
    def defer(self, job_id: str, reason: str, *, execute: bool = False) -> dict[str, Any]:
        if not execute:
            return {"status": "dry-run", "action": "defer"}
        self._id(job_id)
        self._id(reason)
    ...
    def reconcile(
        ...
        *,
        ...
        if not execute:
            return {"status": "dry-run", "action": "reconcile"}
        ...
        with self._open(write=True, identity=True) as (connection, _):
            ...
            reason = (
                "unknown_external_effects" if effects == "unknown" else "expired_lease_no_effects"
            )
            ...
            return {"status": status, "job_id": attempt["job_id"], "reason": reason}

    def _progress(self, connection: sqlite3.Connection, binding: dict[str, Any]) -> dict[str, Any]:
        counts = dict.fromkeys(STATES, 0)
        counts.update(dict(connection.execute("SELECT status, count(*) FROM jobs GROUP BY status")))
        entry_counts = dict.fromkeys(STATES, 0)
        entry_counts.update(
            dict(connection.execute("SELECT status, count(*) FROM entries GROUP BY status"))
        )
        kinds = {kind: dict.fromkeys(STATES, 0) for kind in (*KINDS, "unknown")}
        ...
        return {
            "version": 1,
            "entries": sum(entry_counts.values()),
            "jobs": sum(counts.values()),
            "states": counts,
            "entry_states": entry_counts,
            "kinds": kinds,
            ...
            "coverage": {
                "scope": "supplied_export_only",
                "export_sha256": binding["export_sha256"],
                "all_ordinals_accounted": True,
                "live_sonar_verified": False,
            },
            "debt_reduction": None,
            "tokens": None,
            "cost": None,
    ...
    def document(self, *, execute: bool = False) -> dict[str, Any]:
        if not execute:
            return {"status": "dry-run", "action": "document"}
            ...
            report["attempts"] = [
                {
                    ...
                    "job_id": a["job_id"],
                    "status": a["status"],
                    "context_fingerprint": a["fingerprint"],
                    "context_sha256": a["context_sha"],
                    "context_path": (self._attempt_folder(a) / "job.json")
                    .relative_to(self.state)
                    .as_posix(),
                    "result_sha256": a["result_sha"],
                    "result_path": (self._attempt_folder(a) / "result.json")
                    .relative_to(self.state)
                    .as_posix()
                    if a["result_sha"]
                    else None,
