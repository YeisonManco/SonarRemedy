...
def active_attempts(work):
    with work._open() as (connection, _):
        ...
        return [dict(status=row['job_status'], kind=row['kind'], job_id=row['job_id'], attempt_id=row['id'],
                     lease=row['lease'], context_fingerprint=row['fingerprint'], expires_at=row['expires'],
                     context_path=str(work._attempt_folder(row) / 'job.json')) for row in rows]


def _invalid_result(receipt):
    return dict(version=1, **{k: receipt[k] for k in ('job_id', 'attempt_id', 'lease', 'context_fingerprint')},
                status='failed', edits=[], reason='invalid_manual_proposal', risks=[],
                test_plan='No behavioral proof; the script rejected the supplied proposal.')


def _record_native_failure(work, receipt, reason):
    failure = _invalid_result(receipt)
    failure['reason'] = reason if isinstance(reason, str) and q.TOKEN.fullmatch(reason) else 'native_proposal_failed'
    failure['test_plan'] = 'No behavioral proof; the native proposal process failed or was rejected.'
    return work.complete(failure, execute=True)


def _bind_profile(work, profile):
    data = q.encoded(profile)
    sha = q.digest(data)
    with work._open(write=True, identity=True) as (connection, _):
        existing = connection.execute("SELECT value FROM meta WHERE key='native_profile'").fetchone()
        if existing:
            if existing[0] != sha or q.read_bytes(work.state/'native-profile.json', q.MAX_CONTEXT) != data:
...
def _native_batch(work, receipts, profile, acceptance, native_runner, control_root, deadline):
    ...
    with executor.TargetBarrier(root, control_root) as barrier:
        ...
        try:
            with ThreadPoolExecutor(max_workers=len(receipts)) as pool:
                futures = {pool.submit(dispatch, profile, receipt, acceptance=acceptance,
                                       process_runner=native_runner, deadline=deadline, cancel_event=cancel): receipt
                ...
                try:
                    for future in as_completed(futures):
                        ...
                        try:
                            ...
                            results[receipt['attempt_id']] = dict(status='failed', reason='native_dispatch_failed')
            ...
            try:
                ...
                raise q.Blocked('proposal_worker_target_contamination')
    return results


def _run_native(work, profile, *, resume, integrate, limit, max_batches, deadline,
                native_runner, process_runner, control_root):
    ...
    if not existing and not work.next(limit):
        return dict(status='quiescent', provider='claude', batches=0, processed=0)
    with work._open() as (connection, _):
    ...
    def outcome(status, **fields):
        return dict(status=status, provider='claude', batches=batches, processed=processed, **fields)
    ...
    while True:
        ...
        if not active and not pending:
            return outcome('proposals_ready' if ready else 'quiescent', proposals_ready=len(ready),
        ...
        try:
            results = _native_batch(work, leased, profile, acceptance, native_runner, control_root, deadline) if leased else {}
        ...
        for receipt in active:
            if receipt['status'] == 'leased':
                ...
                try:
                    recorded = (work.complete(result['proposal'], execute=True) if result['status'] == 'completed'
            ...
            if integrate:
                if time.monotonic() >= deadline:
                    return outcome('wall_clock_limit')
                result = executor.integrate(work, receipt['job_id'], execute=True, control_root=control_root,
                                            process_runner=process_runner, deadline=deadline)
...
def run(work, *, provider='manual', execute=False, resume=False, integrate=False,
        limit=4, max_batches=10, wall_seconds=3600, inbox=None,
        proposal_factory=None, process_runner=None, control_root=None,
        profile=None, native_process_runner=None):
    """The optional factory/check callbacks are local test seams, not CLI providers."""
    start = time.monotonic()
    if (type(limit) is not int or not 1 <= limit <= 8 or type(max_batches) is not int or not 1 <= max_batches <= 1000
            or type(wall_seconds) not in (int, float) or not math.isfinite(wall_seconds) or not 0 < wall_seconds <= 86400):
        raise q.Blocked('invalid_runner_bounds')
    if not execute:
        return dict(status='dry-run', provider=provider, limit=limit, max_batches=max_batches,
                    wall_seconds=wall_seconds, integrate=integrate, notice='No files or processes; queue resume only.')
    available = capability(provider, profile)
    if available['status'] != 'available':
        return available
    if provider != 'manual':
        return _run_native(work, profile, resume=resume, integrate=integrate, limit=limit,
                           max_batches=max_batches, deadline=start+wall_seconds,
                           native_runner=native_process_runner, process_runner=process_runner, control_root=control_root)
    existing = active_attempts(work)
    if existing and not resume:
        raise q.Blocked('resume_required_for_existing_attempts')
    with work._open() as (connection, binding):
        if integrate and not connection.execute("SELECT 1 FROM meta WHERE key='executor'").fetchone():
            raise q.Blocked('reviewed_execution_config_required')
    ...
    def outcome(status, **fields):
        return dict(status=status, provider='manual', batches=batches, processed=completed, **fields)
    ...
    while True:
        ...
        if ready:
            return outcome('proposals_ready', jobs=ready, notice='Integrate only after explicit check configuration/review.')


def watch(work, *, interval=2, duration=60):
    if (type(interval) not in (int, float) or not math.isfinite(interval) or not .01 <= interval <= 60
            or type(duration) not in (int, float) or not math.isfinite(duration) or not 0 < duration <= 3600):
        raise q.Blocked('invalid_watch_bounds')
    deadline = time.monotonic()+duration
    sample = 0
    while time.monotonic() < deadline:
        sample += 1
        yield dict(status='watch', sample=sample,
                   progress=work.monitor(timeout=max(.001, min(.5, deadline-time.monotonic()))))
