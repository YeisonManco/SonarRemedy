...
PROFILE_FIELDS = {'version', 'name', 'provider', 'executable', 'executable_sha256', 'cli_version',
                  'model', 'destination', 'auth_env', 'timeout_seconds', 'max_budget_usd'}
PROPOSAL_PROMPT = (
...
def capability(provider, profile=None):
    if provider == 'manual':
        if profile is not None:
            return dict(provider=provider, status='unavailable', reason='profile_provider_mismatch')
        return dict(provider=provider, status='available', reason='offline_proposal_files_only')
    reasons = dict(opencode='opencode_startup_auth_and_dependency_loading_not_isolated',
                   codex='codex_tool_free_startup_unverified', copilot='copilot_tool_free_startup_unverified')
    if provider != 'claude':
        return dict(provider=provider, status='unavailable', reason=reasons.get(provider, 'unknown_provider'))
    try:
        validate_profile(profile)
    except (q.Blocked, OSError, ValueError, TypeError) as error:
        return dict(provider=provider, status='unavailable',
                    reason=str(error) if isinstance(error, q.Blocked) else 'profile_runtime_unavailable')
    return dict(provider=provider, status='available', profile=CLAUDE_PROFILE,
                reason='explicit_bare_api_key_profile_requires_local_preflight', live_verified=False)
...
def validate_profile(profile):
    if (not isinstance(profile, dict) or set(profile) != PROFILE_FIELDS
            ...
            or profile['name'] != CLAUDE_PROFILE or profile['provider'] != 'claude'
            or profile['auth_env'] != 'ANTHROPIC_API_KEY' or profile['destination'] != CLAUDE_DESTINATION
            or not isinstance(profile['model'], str) or not re.fullmatch(r'claude-[a-z0-9.-]{3,100}', profile['model'])
            or not isinstance(profile['cli_version'], str) or not re.fullmatch(r'\d+\.\d+\.\d+', profile['cli_version'])
            or tuple(map(int, profile['cli_version'].split('.'))) < (2, 1, 139)
            or type(profile['timeout_seconds']) is not int or not 1 <= profile['timeout_seconds'] <= 1200
            or type(profile['max_budget_usd']) not in (int, float) or not math.isfinite(profile['max_budget_usd'])
            or not 0 < profile['max_budget_usd'] <= 25):
        raise q.Blocked('invalid_claude_profile')
    if (not isinstance(profile['executable'], str) or not Path(profile['executable']).is_absolute()
...
def claude_invocation(profile, context, runtime_dir, session_id, acceptance=None):
    ...
    payload = q.encoded(dict(job=context, acceptance=acceptance or {}))
    if len(payload) + len(PROPOSAL_PROMPT.encode()) > q.MAX_CONTEXT:
        raise q.Blocked('native_context_budget_exceeded')
    argv = [profile['executable'], '--bare', '-p', '--tools', '', '--disable-slash-commands',
            '--strict-mcp-config', '--mcp-config', '{"mcpServers":{}}', '--no-session-persistence',
            '--no-chrome', '--output-format', 'json', '--model', profile['model'],
            '--session-id', session_id, '--max-budget-usd', str(profile['max_budget_usd']),
            '--system-prompt', PROPOSAL_PROMPT]
    return dict(argv=argv, cwd=str(runtime_dir), input_data=payload)


def _runtime_environment(profile, runtime_dir, *, authentication):
    env = dict(ANTHROPIC_BASE_URL=profile['destination'], CLAUDE_CONFIG_DIR=str(runtime_dir),
...
def preflight(profile, cwd, *, process_runner=None, deadline=None):
    try:
        validate_profile(profile)
        ...
        for flag in ('--version', '--help'):
            ...
            result = runner([profile['executable'], '--bare', flag], runtime, timeout=min(10, remaining),
                            output_limit=64*1024, explicit_env=env)
            if result.get('status') != 'exited' or result.get('exit_code') != 0 or result.get('reason'):
        ...
        required = ('--bare', 'ANTHROPIC_API_KEY', '--tools', '--strict-mcp-config', '--mcp-config',
                    '--disable-slash-commands', '--no-session-persistence', '--output-format',
                    '--model', '--session-id', '--max-budget-usd', '--no-chrome', '--system-prompt')
        ...
        if not os.environ.get(profile['auth_env']):
            raise q.Blocked('authorized_auth_env_missing')
        return dict(status='available', profile_sha256=q.digest(q.encoded(profile)), cli_version=profile['cli_version'])
    except (q.Blocked, OSError, ValueError, TypeError, UnicodeError) as error:
        return dict(status='unavailable', reason=str(error) if isinstance(error, q.Blocked) else 'native_preflight_failed')


def dispatch(profile, receipt, *, acceptance=None, process_runner=None, deadline=None, cancel_event=None):
    ...
    if marker.exists():
        ...
        if saved.get('profile_sha256') != profile_sha or saved.get('context_fingerprint') != receipt['context_fingerprint']:
            raise q.Blocked('native_dispatch_binding_mismatch')
        if not outcome_file.exists():
            return dict(status='interrupted', reason='native_dispatch_reconciliation_required')
        stored = q.parse_json(q.read_bytes(outcome_file, 2*q.MAX_RESULT))
        if (stored.get('sha256') != q.digest(q.encoded(stored.get('payload')))
    ...
    if (q.digest(q.encoded(fingerprinted)) != receipt['context_fingerprint']
            or any(context.get(k) != receipt[k] for k in ('job_id', 'attempt_id', 'lease', 'context_fingerprint'))):
    ...
    runtime.mkdir()
    data = dict(job_id=receipt['job_id'], attempt_id=receipt['attempt_id'], session_id=session,
                profile_sha256=profile_sha, context_fingerprint=receipt['context_fingerprint'],
    ...
    try:
        ...
        remaining = (time.monotonic()+profile['timeout_seconds'] if deadline is None else deadline)-time.monotonic()
        ...
        runner = process_runner or run_process
        result = runner(invocation['argv'], invocation['cwd'], input_data=invocation['input_data'],
                        timeout=timeout, explicit_env=env, cancel_event=cancel_event)
        if result.get('reason') or result.get('status') != 'exited':
            raise q.Blocked(result.get('reason') or 'native_process_failed')
        proposal = parse_events('claude', result['stdout'], session_id=session, exit_code=result['exit_code'],
                                sensitive_values=(env['ANTHROPIC_API_KEY'],))
        if any(proposal.get(k) != receipt[k] for k in ('job_id', 'attempt_id', 'lease', 'context_fingerprint')):
            raise q.Blocked('native_proposal_identity_mismatch')
        outcome = dict(status='completed', session_id=session, proposal=proposal)
        ...
        outcome = dict(status='failed', session_id=session, reason=reason)
    ...
    q.write_immutable(outcome_file, q.encoded(dict(payload=outcome, sha256=q.digest(q.encoded(outcome)))))
    return outcome


def parse_events(provider, data, *, session_id, exit_code, sensitive_values=()):
    if provider not in ('opencode', 'codex', 'claude'):
        raise q.Blocked('native_envelope_or_tool_trace_unverified')
    if type(exit_code) is not int or exit_code != 0 or not isinstance(data, bytes) or len(data) > 1024 * 1024 or not session_id:
        raise q.Blocked('native_exit_or_budget_invalid')
    variants = {variant.encode('utf-8') for value in sensitive_values if value for variant in
    ...
    if provider == 'claude':
        ...
        if (not isinstance(envelope, dict) or envelope.get('type') != 'result' or envelope.get('subtype') != 'success'
                or envelope.get('is_error') is not False or envelope.get('session_id') != session_id
                or type(envelope.get('num_turns')) is not int or envelope['num_turns'] != 1
                or envelope.get('stop_reason', 'end_turn') != 'end_turn'
                or any(envelope.get(k) for k in ('tool_calls', 'tool_results', 'permission_denials',
                                                'deferred_tool_use', 'structured_output'))):
            raise q.Blocked('claude_terminal_session_or_tool_trace_invalid')
        def tool_trace(value):
            if isinstance(value, dict):
                return (value.get('type') in ('tool_use', 'tool_result', 'mcp_tool_use')
                        or any(k.endswith('_requests') and isinstance(v, (int, float)) and v > 0
                               for k, v in value.items() if k in ('web_search_requests', 'web_fetch_requests'))
    ...
    if provider == 'opencode':
        if kinds != ['step_start', 'text', 'step_finish'] or any(e.get('sessionID') != session_id for e in events):
        ...
        if (kinds != ['thread.started', 'turn.started', 'item.completed', 'turn.completed']
...
def run_process(argv, cwd, *, timeout=30, output_limit=1024 * 1024, input_data=b'', explicit_env=None, cancel_event=None):
    ...
    allowed_env = {'ANTHROPIC_API_KEY', 'ANTHROPIC_BASE_URL', 'CLAUDE_CONFIG_DIR',
                   'DISABLE_AUTOUPDATER', 'CLAUDE_CODE_DISABLE_NONESSENTIAL_TRAFFIC'}
    if explicit_env is not None and (not isinstance(explicit_env, dict) or set(explicit_env)-allowed_env
    ...
    if (os.name != 'nt' or type(timeout) not in (int, float) or not math.isfinite(timeout)
            or not 0 < timeout <= 1800 or type(output_limit) is not int or not 1 <= output_limit <= 1024 * 1024
            or not isinstance(input_data, bytes) or len(input_data) > q.MAX_CONTEXT):
    ...
    if (not isinstance(argv, list) or not 1 <= len(argv) <= 64
            ...
            or not Path(argv[0]).is_absolute() or Path(argv[0]).suffix.lower() != '.exe'):
    ...
    try:
        ...
        process, thread, _, _ = _winapi.CreateProcess(str(executable), subprocess.list2cmdline(argv),
            None, None, True, 0x4 | subprocess.CREATE_NO_WINDOW, child_env, str(cwd), startup)
        ...
        readers = [threading.Thread(target=drain, args=(stdout_r, True), daemon=True),
        ...
        if any(reader.is_alive() for reader in readers):
            ...
            reason = 'process_output_limit'
        return dict(status='failed' if reason else 'exited', reason=reason, exit_code=exit_code,
                    stdout=bytes(output), elapsed_seconds=time.monotonic() - start)
