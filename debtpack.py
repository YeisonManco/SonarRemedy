...
MAX_CONCURRENT = 8
MAX_ISSUES = 3000
IGNORE = {'.git', '.env', '.ssh', '.aws', '.azure', 'secrets', 'credentials',
          'node_modules', '.venv', 'vendor', 'bin', 'obj', '__pycache__'}
MANIFESTS = {'package.json', 'pyproject.toml', 'requirements.txt', 'pom.xml',
             'build.gradle', 'go.mod', 'Cargo.toml', 'sonar-project.properties',
...
TOKEN_PLAIN_SUFFIXES = ('.txt', '.env', '.json', '.config', '.ini', '.xml',
                        '.yaml', '.yml', '.properties', '.csv')


def hidden_secret(part):
    name = part.lower()
    return (name in IGNORE or name.startswith('.env') or
            any(word in name for word in ('secret', 'credential')) or
            name in ('token', 'tokens') or
            ('token' in name and name.endswith(TOKEN_PLAIN_SUFFIXES)) or
    ...
    for base, dirs, files in os.walk(root, followlinks=False):
        ...
        for name in sorted(files):
            ...
            if hidden_secret(name) or linked(Path(base) / name):
                continue
            if (name in MANIFESTS or name.endswith(('.csproj', '.sln')) or
                    (rel.startswith('.github/workflows/') and name.endswith(('.yml', '.yaml')))):
                safe_path(root, rel)
                paths.append(rel)
    proposals = []
    for path in sorted(paths):
        name = Path(path).name
        command = ('python -m unittest discover' if name == 'pyproject.toml' else
                   'npm test' if name == 'package.json' else
                   'dotnet test' if name.endswith(('.csproj', '.sln')) else None)
        if command:
            proposals.append({'manifest': path, 'cwd': str(Path(path).parent),
                              'candidate': command, 'verified': False})
    return {'paths': sorted(paths), 'proposals': proposals,
...
def plan(root, export, limit=4):
    if type(limit) is not int or not 1 <= limit <= 8:
        raise ValueError('task limit must be 1..8')
    if (not isinstance(export, dict) or type(export.get('version')) is not int or
            export['version'] != 1 or not token(export.get('revision'))):
    ...
    if not isinstance(issues, list) or len(issues) > MAX_ISSUES:
        raise ValueError('issues must be a list of at most %d items' % MAX_ISSUES)
    tasks, seen = [], set()
    for item in issues:
        ...
        seen.add(item['id'])
        if item.get('kind') not in KINDS or type(item.get('line')) is not int or not 1 <= item['line'] <= 10000000:
        ...
        identity = json.dumps([export['revision'], item['id'], item['kind'], item['path'], item['line']])
        tasks.append({'id': hashlib.sha256(identity.encode()).hexdigest()[:16],
                      'issue': item['id'], 'kind': item['kind'], 'path': item['path'],
                      'line': item['line'], 'status': 'planned'})
    tasks.sort(key=lambda t: (t['kind'], t['path'], t['line'], t['issue']))
    return {'version': 1, 'target': str(Path(root).resolve()), 'revision': export['revision'],
            'remaining': max(0, len(tasks) - limit), 'tasks': tasks[:limit], 'results': {}}
...
def context(state, task_id):
    ...
    result = {'revision': state['revision'], 'target': state['target'], 'task': task,
            'contract': 'prompts/' + task['kind'] + '.md',
            'budget': {'context_bytes': 8192, 'changed_files': 4, 'attempts': 2},
            'parallel': True, 'max_concurrent': MAX_CONCURRENT,
...
def transition(state, task_id, status):
    task = get_task(state, task_id)
    allowed = {'planned': {'running'}, 'running': {'blocked', 'submitted'},
               'blocked': {'running'}, 'submitted': {'blocked'}, 'validated': set(), 'accepted': set()}
...
def validate(evidence, revision, now=None, max_age=86400, evidence_root=None):
    ...
    for key in ('build_report', 'tests_report', 'coverage_report', 'sonar_report'):
        try:
            ...
            if evidence_root is not None:
                ...
                if not report.is_file() or report.stat().st_size > 1048576 or report.stat().st_size == 0:
    ...
    for key in ('sonar_timestamp', 'validated_timestamp'):
        ...
        if type(value) not in (int, float) or not math.isfinite(value) or not 0 <= now - value <= max_age:
    ...
    return {'status': 'blocked' if errors else 'validated', 'errors': errors,
...
def main():
    ...
    parser.add_argument('--state', default='.debt-state/session.json', help='relative to pack; never overwritten on plan')
    ...
    check.add_argument('--revision', required=True, help='operator-verified current target revision')
    accept = commands.add_parser('accept-local', help='accept one improvement, not global cleanliness')
    accept.add_argument('task')
    accept.add_argument('baseline')
    accept.add_argument('after')
    accept.add_argument('--allow', action='append', required=True, help='approved relative path; max four')
    ...
    try:
        ...
        if args.command == 'discover':
            ...
            if args.command == 'status':
                ...
                result = validate(read_json(args.evidence), args.revision,
                                  evidence_root=Path(args.evidence).resolve().parent)
                task['status'] = result['status']
                state['results'][args.task] = dict(result, target_revision=args.revision,
