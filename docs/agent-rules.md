...
2. Specialists produce proposals only: one bounded context, fresh session, **no tools** or target edits. The serial script integrator alone applies approved changes and runs checks. Native providers currently remain unavailable; use manual proposal files without bypassing host permissions.
...
9. Rule+path suggests a propagation candidate only. Each extra branch requires explicit approval, divergence analysis and its own queue/evidence. Never copy blindly across divergent branches.
