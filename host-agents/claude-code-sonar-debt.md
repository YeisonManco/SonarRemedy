...
The operator supplies `<PACK>/docs/agent-contract.md` restrictions, `<PACK>/host-agents/sonar-worker.md` and one bounded job context to a demonstrably tool-free fresh session. No tools, target edits, builds/tests, Git, network, skill loading or child agents. Return a strict proposal, never claimed execution evidence.
