...
The operator must provide the canonical `<PACK>/docs/agent-contract.md` restrictions and one bounded `job.json` before the session. Follow `<PACK>/host-agents/sonar-worker.md`: no tools, target access, Git/build/tests/network, skill loading or child agents. Return only structured proposal JSON with exact lease/context identity. If context is insufficient, defer.
