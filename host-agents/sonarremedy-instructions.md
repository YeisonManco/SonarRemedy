...
0. **Resolve the checkout you are standing in** — the pack binds queues to the EXACT git identity (root/branch/revision) of the checkout you pass, so you must pass it explicitly every time. Resolve `git rev-parse --show-toplevel` and `git rev-parse --abbrev-ref HEAD` for the repo you are working in, and use THAT path as `--repo` on every command (the MCP tool schema only exposes `--repo`; `--dir` is a raw-CLI-only flag on `doctor`, not available here). Never rely on the current-directory default or a config `local_path` you have not verified. `doctor` now reports which saved project binds to a checkout (the `project` check), and `_load_config` auto-selects that project when no `--project`/`--config` is given — but only you know which repo you actually mean, so pass it.
...
- **"It ran against the WRONG project/repo" (wrong Sonar project or wrong checkout)** — the pack picked the wrong config because the checkout you are in did not match the project. Two safeguards now exist: `doctor` reports a `project` check (which saved project binds to this checkout: `ok`/`ambiguous`/`warning`), and `_load_config` auto-selects the matching project from the explicit `--repo <path>` you passed (the documented way to run these commands, since the CLI itself runs from the pack directory) when no `--project`/`--config` is passed — falling back to the current directory only if no `--repo` was given. If `doctor` reports `project` as `ambiguous`, pass `--project` explicitly. If it reports `warning` (no match), the checkout has no saved project — run `sonar_remedy_configure_project` for it, or pass `--project`/`--config`. Always pass the checkout path explicitly as `--repo` (see 0b step 0).
- **`project_collisions` warning** — two saved projects share the same `local_path` or `repository.url`, so resolution is ambiguous. Give one of them a distinct `local_path`/URL (`sonar_remedy_configure_project`), or always pass `--project`.
- **`remote_credentials` warning** — the checkout's `origin` remote URL embeds credentials (a PAT baked into the URL). Re-set the remote without the token (`git remote set-url origin <url>`), then let the Git credential manager (or the env `PAT` the config's `pat_env` names) supply auth. Never commit a remote URL with a token in it.
- **`queue_project` info / "not in the queue index"** — `doctor --state <queue>` reports the project registered for that queue in `~/.sonar-remedy/queues.json`. `slice --execute` and `recover` both record it automatically now (including a `--config` file whose own `repository` unambiguously matches a saved project). `info` just means the queue predates the index or was sliced/recovered before registration existed (or its project could not be resolved); it is informational, not an error. To recover a lost project↔queue mapping, run `doctor --state <queue> --repo <path>` and read the `project` + `queue_identity` checks.
