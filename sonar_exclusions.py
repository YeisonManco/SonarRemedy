...
MAX_VISITED = 20000
MAX_BYTES = 1024 * 1024
CODE_EXTENSIONS = (".cs", ".vb", ".ts", ".tsx", ".js", ".jsx", ".mjs", ".cjs", ".cshtml", ".razor")
...
def empty_counts() -> dict[str, Any]:
    return {
        "nosonar": 0,
        "nosonar_rule_specific": 0,
        "pragma_suppress": 0,
        "suppress_message": 0,
        "exclude_from_coverage": 0,
        "sonarqube_exclude": 0,
        "properties_exclusions": {
            "sonar_exclusions": 0,
            "sonar_coverage_exclusions": 0,
...
def scan_source(path: Path, counts: dict[str, Any]) -> bool | None:
...
def scan_csproj(path: Path, counts: dict[str, Any]) -> bool | None:
...
def scan_properties(path: Path, counts: dict[str, Any]) -> bool | None:
...
def main() -> int:
    ...
    parser.add_argument(
        "--output", help="optional JSON report path outside the repo; default prints to stdout only"
    )
