...
from debtpack import PACK, read_json, token, MAX_ISSUES
...
class Config:
    def __init__(self, raw):
        allowed = {'adapter', 'repo', 'url', 'trusted_url', 'project', 'branch', 'timeout', 'severities',
                   'exclude_ids', 'allow_http'}
...
def git_dirty(repo):
    ...
    result = subprocess.run(['git', '-C', str(repo), 'status', '--porcelain'], capture_output=True,
                            timeout=30, env=child_environment(), check=False)
    return result.returncode == 0 and bool(result.stdout.strip())


def measures(get, project, branch):
...
def chunk_issues(normalized, chunk_size):
...
def fetch(config, output):
    ...
    revision = None
    try:
        data = get('api/project_analyses/search', project=config.project, branch=config.branch, ps=1)
        ...
        analysis_warning = 'analysis lookup blocked (%s); using local HEAD as revision' % str(error)
    ...
    if revision is None:
        ...
        raise Blocked('checkout must be at the analyzed revision; HEAD %s does not match analysis %s'
                      % (head[:12], revision[:12]))
    gate = get('api/qualitygates/project_status', projectKey=config.project,
               branch=config.branch).get('projectStatus', {}).get('status')
    if gate not in ('OK', 'ERROR'):
        raise Blocked('quality gate unavailable')
    found = measures(get, config.project, config.branch)
    issue_params = dict(componentKeys=config.project, branch=config.branch, resolved='false')
    ...
    try:
        hotspots = pages(get, 'api/hotspots/search', 'hotspots', dict(projectKey=config.project,
                                                                      branch=config.branch))
    except (Blocked, HTTPError) as error:
        hotspots_warning = 'hotspot lookup blocked (%s); unreviewed hotspots unknown' % str(error)
        hotspots = []
    normalized = []
    deferred_no_line = 0
    for item in issues:
    ...
    for idx, chunk in enumerate(chunks):
        path = (output / 'export.json') if len(chunks) == 1 else (output / ('chunk-%d.json' % idx))
        path.write_text(json.dumps(dict(version=1, revision=revision, issues=chunk), indent=2),
                        encoding='utf-8')
    ...
    warnings = []
    if excluded_count:
        warnings.append('excluded %d explicitly configured issue(s)' % excluded_count)
    if deferred_no_line:
        warnings.append('deferred %d issue(s) without file/line (manual triage required)' % deferred_no_line)
    if git_dirty(config.repo):
        warnings.append('working tree is not clean; export binds the analyzed revision, not uncommitted edits')
    # Compact only: never leak issue lists or raw evidence into model context.
    result = dict(status='collected', export=str(export_path), project=config.project,
                  branch=config.branch, revision=revision, gate=gate,
                  exports=[str(p) for p in export_paths], chunks=len(chunks),
                  measures=dict(coverage=found['coverage'], duplication=found['duplicated_lines_density'],
                                smells=found['code_smells'], vulnerabilities=found['vulnerabilities'],
                                hotspots=found['security_hotspots']),
                  issues_total=len(normalized), issues_by_kind=kinds,
                  deferred_no_line=deferred_no_line,
                  unreviewed_hotspots=unreviewed, warnings=warnings)
...
def main():
    ...
    try:
        ...
        secret = os.environ.get('SONAR_TOKEN', '')
        reason = redactor(secret)(str(error)) if isinstance(error, Blocked) else type(error).__name__
