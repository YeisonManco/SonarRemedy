...
class Config:
    def __init__(self, raw):
        allowed = {'adapter', 'repo', 'solution', 'url', 'trusted_url', 'project', 'branch', 'timeout', 'poll_attempts'}
        if not isinstance(raw, dict) or set(raw) - allowed:
            raise Blocked('unsupported configuration field; credentials and skip switches forbidden')
...
def redactor(secret):
    ...
    def redact(text):
        ...
        return re.sub(r'(?i)(authorization\s*[:=]\s*(?:bearer|basic)\s+|sonar\.(?:token|login)\s*=)[^\s"<>]+',
                      r'\1[REDACTED]', text)
    return redact
...
def child_environment():
    ...
    return {k: v for k, v in os.environ.items()
            if not k.upper().startswith(forbidden) and k.upper() not in ('HTTP_PROXY', 'HTTPS_PROXY', 'ALL_PROXY')}


def snapshot(repo):
    def git(*args):
        result = subprocess.run(['git', '-C', str(repo), *args], capture_output=True,
                                timeout=30, env=child_environment(), check=False)
    ...
    for name in names:
        ...
        if any(p in ('.sonarqube', '.debt-runs', '.debt-scan.lock', 'bin', 'obj') for p in Path(name).parts):
            continue
        path = repo / name
    ...
    return dict(revision=revision, digest=digest.hexdigest(), diff_digest=hashlib.sha256(diff).hexdigest(), files=files)


def run_process(argv, cwd, env, timeout, log, redact):
    ...
    try:
        ...
        proc.stdout.close()
        log.write_text(redact(output.decode('utf-8', errors='replace')) or '(no process output)\n', encoding='utf-8')


def fresh(path, started):
    if (not path.is_file() or linked(path) or not 0 < path.stat().st_size <= 64 * 1024 * 1024
            or path.stat().st_mtime < started - 2):
        raise Blocked('report missing, stale, linked, empty or exceeds budget')
    return path.read_bytes()


def test_reports(folder, started):
    reports = sorted(folder.rglob('*.trx'))
    if not reports or len(reports) > 100:
        raise Blocked('test reports unavailable or exceed budget')
    counts = dict(total=0, executed=0, passed=0, failed=0, notExecuted=0)
    for report in reports:
        ...
        if (values['total'] != values['executed']
                or values['passed'] != values['total'] or values['notExecuted']):
    ...
    for report in coverage:
        ...
        if root.tag != 'CoverageSession' or summary is None or int(summary.get('numSequencePoints', '0')) <= 0:
            raise Blocked('OpenCover has no instrumented sequence points')
    return counts


def task_report(repo, started, allowed_url, project, previous):
    ...
    if (fields.get('projectKey') != project or fields.get('serverUrl', '').rstrip('/') != allowed_url
            or (parsed.scheme, parsed.netloc, parsed.path) != (expected.scheme, expected.netloc, expected.path)
            or parsed.fragment or parse_qs(parsed.query) != {'id': [task]} or task == previous):
        raise Blocked('scanner report identity, endpoint or freshness mismatch')
    return task


def scan(config, output, execute=False, runner=run_process):
    ...
    if not execute:
        return dict(status='dry-run', adapter='dotnet', repo=str(config.repo), solution=str(config.solution),
                    project=config.project, branch=config.branch, url=config.url,
                    stages=['scanner-begin', 'build', 'tests/OpenCover', 'scanner-end/upload', 'CE/collection'],
    ...
    run.mkdir()
    started = time.time()
    result = dict(status='blocked', stage='snapshot', run=str(run), started=started, tests=None, reports={})
    ...
    try:
        ...
        if old_report.is_file():
            previous = dict(line.split('=', 1) for line in old_report.read_text().splitlines() if '=' in line).get('ceTaskId')
        client = Client(config.url, secret, min(config.timeout, 60))
        with Gateway(client) as gateway:
            auth = '/d:sonar.token=' + gateway.credential
            commands = [
                ('scanner-begin', ['dotnet', 'sonarscanner', 'begin', '/k:' + config.project, auth,
                                  '/d:sonar.host.url=' + gateway.url, '/d:sonar.branch.name=' + config.branch,
                                  '/d:sonar.scm.revision=' + before['digest'],
                                  '/d:sonar.scanner.skipJreProvisioning=true',
                                  '/d:sonar.cs.opencover.reportsPaths=' + str(results / '**/coverage.opencover.xml')]),
                ('build', ['dotnet', 'build', str(config.solution), '--no-incremental', '--disable-build-servers']),
                ('tests', ['dotnet', 'test', str(config.solution), '--no-build', '--no-restore',
                           '--logger', 'trx', '--results-directory', str(results), '--collect:XPlat Code Coverage',
                           '--', 'DataCollectionRunSettings.DataCollectors.DataCollector.Configuration.Format=opencover']),
                ('scanner-end', ['dotnet', 'sonarscanner', 'end', auth])]
            for stage, argv in commands:
                result['stage'] = stage
                code = runner(argv, config.repo, child_environment(), config.timeout, run / (stage + '.log'), redact)
        ...
        sonar = collect(client.get, config.project, config.branch, task, before['digest'], started,
                        attempts=config.poll_attempts)
        if snapshot(config.repo) != before:
            raise Blocked('source snapshot changed during scan')
        result['sonar'] = sonar
        result['status'] = 'collected'
        export = dict(version=1, revision=before['digest'], issues=sonar['issues'])
        ...
        result['reason'] = redact(str(error)) if isinstance(error, Blocked) else type(error).__name__ + ': execution/evidence unavailable'
        ...
        for file in sorted(run.rglob('*')):
            if file.is_file() and not linked(file):
                if file.stat().st_size > 64 * 1024 * 1024:
                    ...
                    result['reason'] = 'report exceeds retention budget; inspect locally before sharing'
                    continue
                raw = file.read_bytes()
                clean = redact(raw.decode('utf-8', errors='replace')).encode('utf-8')
                file.write_bytes(clean)
                result['reports'][file.relative_to(run).as_posix()] = hashlib.sha256(clean).hexdigest()
        result['finished'] = time.time()
        (run / 'evidence.json').write_text(redact(json.dumps(result, indent=2)), encoding='utf-8')


def verify_run(run, repo, current=True):
    run = Path(run).resolve(strict=True)
    evidence = read_json(run / 'evidence.json')
    if (evidence.get('status') != 'collected' or not 0 <= time.time() - evidence.get('finished', 0) <= 86400
            or evidence.get('tests', {}).get('total', 0) <= 0
            or evidence.get('tests', {}).get('failed') != 0
            or any(evidence.get('return_codes', {}).get(k) != 0 for k in ('scanner-begin', 'build', 'tests', 'scanner-end'))):
        raise Blocked('run unavailable, stale, incomplete or failed')
    reports = evidence.get('reports', {})
    if not {'build.log', 'tests.log', 'scanner-begin.log', 'scanner-end.log', 'sonar.json', 'export.json'} <= set(reports):
    ...
    retained = {p.relative_to(run).as_posix() for pattern in ('*.trx', 'coverage.opencover.xml')
...
def job_improved(kind, issue, before, after):
...
def accept_job(state, task, baseline, after, allowed):
    ...
    return dict(status='accepted', global_pass=current['sonar']['global_pass'], changed_files=changed,
                before=str(baseline), after=str(after),
...
def main():
    ...
    try:
        ...
        print(json.dumps({k: v for k, v in result.items() if k in ('status', 'stage', 'run', 'reason', 'notice', 'stages')}, indent=2))
