...
if '--version' in sys.argv:
    ...
    raise SystemExit(0)
if '--help' in sys.argv:
    print('--bare ANTHROPIC_API_KEY --tools --disable-slash-commands --strict-mcp-config '
...
if Path(path).stem.startswith('f') and Path(path).stem[1:].isdigit():
    delay = .05 * (3 - int(Path(path).stem[1:]) % 4)
time.sleep(delay)
proposal = dict(version=1, job_id=job['job_id'], attempt_id=job['attempt_id'], lease=job['lease'],
                context_fingerprint=job['context_fingerprint'], status='proposed', reason='', risks=[],
                test_plan='Synthetic fixture; configured characterization checks remain external.',
                edits=[dict(path=path, before_sha256=job['sources'][0]['sha256'],
                            replacements=[dict(old='Value()', new='Value( )')])])
if 'partial' in path:
...
print(json.dumps(dict(type='result', subtype='success', is_error=False, session_id=session,
                      num_turns=1, stop_reason='end_turn', result=json.dumps(proposal),
                      _fixture_start_ns=started, _fixture_end_ns=time.time_ns(),
