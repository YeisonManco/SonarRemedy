...
class CliSmoke(unittest.TestCase):
            ...
            json.dumps(
                {
                    "version": 1,
                    "revision": "base123",
                    "issues": [{"id": "S1", "kind": "coverage", "path": "a.py", "line": 1}],
                }
            ),
            encoding="utf-8",
        )
    ...
    def test_end_to_end_missing_evidence_blocked(self):
        ...
        self.assertEqual(
            self.run_cli("validate", job, evidence, "--revision", "new123", code=2)["status"],
            "blocked",
        )
        self.assertEqual(self.run_cli("status")["tasks"][0]["status"], "blocked")
        self.assertEqual(
            before,
            {p.name: hashlib.sha256(p.read_bytes()).hexdigest() for p in self.target.iterdir()},
        )
