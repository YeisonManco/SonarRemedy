...
class ContractTests(unittest.TestCase):
    def test_canonical_entrypoints_reference_queue_and_proposal_boundary(self):
        for name in ('AGENTS.md', 'README.md', 'docs/agent-contract.md', 'docs/agent-rules.md', 'docs/validation.md'):
        ...
        for folder in ('host-agents', 'bridges'):
            for path in (PACK/folder).glob('*.md'):
                ...
                with self.subTest(path=path.name):
                    ...
                    self.assertIn('proposal', text)
                    self.assertNotIn('worker reads only that file and nearby symbols, writes a failing test', text)
