...
class NativeTests(QueueFixture):
    ...
    def native_process(self, argv, cwd, **kwargs):
        ...
        result = t.run_process(
            [sys.executable, "-B", str(PACK / "tests/fake_claude.py"), *argv[1:]], cwd, **kwargs
        )
    ...
    def native_run(self, **kwargs):
        with patch.dict(
            os.environ,
            {"ANTHROPIC_API_KEY": "fixture-approved-key", "SONAR_TOKEN": "fixture-sonar-private"},
        ):
            return r.run(
                self.queue(),
                provider="claude",
                ...
                execute=True,
                native_process_runner=self.native_process,
                **kwargs,
            )
    ...
    def test_real_parallel_dispatch_and_no_integration_before_all_workers_finish(self):
        ...
        def check(argv, cwd, **kwargs):
            self.assertEqual(
                len(self.intervals), 2, "all proposal children must finish before any check"
            )
            return cli_fixtures.RunnerTests.check(self, argv, cwd, **kwargs)

        result = self.native_run(
            limit=2, integrate=True, process_runner=check, control_root=self.home / "control"
        )
        self.assertEqual(result["status"], "quiescent")
        self.assertEqual(self.queue().monitor()["states"]["locally_verified"], 2)
        self.assertLess(
            max(a for a, b in self.intervals),
            min(b for a, b in self.intervals),
            "real children must overlap",
        )
    ...
    def test_1205_findings_real_process_batches_resume_without_duplicate_effects(self):
        ...
        options = {
            "limit": 3,
            "integrate": True,
            "process_runner": lambda *a, **kw: cli_fixtures.RunnerTests.check(self, *a, **kw),
    ...
    def test_watch_is_bounded_read_only_and_no_source(self):
        self.create()
        before = self.files()
        samples = list(r.watch(self.queue(), interval=0.01, duration=0.05))
