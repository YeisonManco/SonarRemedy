...
class QueueFixture(unittest.TestCase):
    ...
    def write_export(self, entries):
        self.export.write_text(json.dumps(dict(version=1, revision=REVISION, issues=entries)), encoding='utf-8')

    def identity(self, target):
        return dict(root=str(target), branch='feature/queue', revision=REVISION)

    def create(self, **kwargs):
        return q.slice_queue(self.target, self.export, self.state, 'feature/queue',
                             execute=True, identity_reader=self.identity, **kwargs)
    ...
    def proposal(self, receipt):
        ...
        return dict(version=1, job_id=receipt['job_id'], attempt_id=receipt['attempt_id'],
                    lease=receipt['lease'], context_fingerprint=receipt['context_fingerprint'],
                    status='proposed', edits=[dict(path='a.cs', before_sha256=context['sources'][0]['sha256'],
                        replacements=[dict(old='=> 1', new='=> 3')])], reason='', risks=[],
...
class IntakeTests(QueueFixture):
    ...
    def test_duplicates_and_case_aliases_defer_all_affected_entries(self):
        self.write_export([self.issue(), self.issue(path='b.cs'),
                           self.issue('S3', 'A.cs'), self.issue('S4', 'b.cs', 'coverage')])
    ...
    def test_every_unusable_entry_is_visible_without_raw_messages(self):
        ...
        missing.pop('rule')
        self.write_export([None, missing, self.issue('S3', '../escape.cs'),
                           self.issue('S4', 'missing.cs'), self.issue('S5', kind='hotspots'),
    ...
    def test_slice_dry_run_never_calls_identity_or_creates_state(self):
        result = q.slice_queue(self.target, self.export, self.state, 'feature/queue',
...
class LifecycleTests(QueueFixture):
    ...
    def test_empty_metric_only_export_cannot_claim_debt_reduction(self):
        self.export.write_text(json.dumps(dict(version=1, revision=REVISION, issues=[], coverage=50)))
        self.create()
    ...
    def test_actual_repeated_old_text_is_rejected(self):
        ...
        proposal['edits'][0]['replacements'] = [dict(old='int', new='long')]
    ...
    def test_replacement_outside_retained_window_is_rejected(self):
        ...
        receipt = self.leased()
        proposal = self.proposal(receipt)
        proposal['edits'][0]['replacements'] = [dict(old='// line 1\n', new='// changed\n')]
    ...
    def test_sqlite_connection_failure_is_a_bounded_blocked_outcome(self):
        self.create()
        caught = None
        with patch.object(q.sqlite3, 'connect', side_effect=sqlite3.OperationalError('fixture locked')):
            try:
                self.queue().monitor()
    ...
    def test_new_result_cannot_exceed_total_artifact_budget(self):
        ...
        with patch.object(q, 'MAX_ARTIFACTS', used + 64):
            with self.assertRaises(q.Blocked):
    ...
    def test_all_state_paths_checked_before_sqlite_opens(self):
        ...
        with patch.object(q, 'local_path', side_effect=check), patch.object(q.sqlite3, 'connect', side_effect=connect):
            with self.assertRaises(q.Blocked):
                self.queue().monitor()
        self.assertEqual(opened, [], 'unsafe journal/state siblings must be rejected before SQLite can inspect them')
    ...
    def test_next_counts_files_and_skips_deferred_priority_stage(self):
        self.write_export([self.issue('H', kind='hotspots'), self.issue('S1'), self.issue('S2'),
                           self.issue('S3', 'b.cs')])
        self.create()
    ...
    def test_same_write_set_locks_across_kind_and_shared_test_path(self):
        self.write_export([self.issue('S1'), self.issue('S2', kind='coverage'), self.issue('S3', 'b.cs')])
    ...
    def test_concurrent_real_process_claims_do_not_duplicate_lease(self):
        self.create()
        code = ('import json,sys; import debt_queue as q; '
        ...
        children = [subprocess.Popen([sys.executable, '-B', '-c', code, str(self.state)], cwd=PACK,
    ...
    def test_completion_rejects_wrong_identity_partial_and_success_flags(self):
        ...
        for field, value in [('lease', 'wrong'), ('job_id', 'wrong'), ('attempt_id', 'wrong'),
                             ('context_fingerprint', '0' * 64), ('status', 'success'),
                             ('status', 'partial'), ('all_fixed', True)]:
    ...
    def test_stale_claim_defers_and_independent_job_continues(self):
        ...
        self.create()
        job = next(j for j in self.queue().next() if j['path'] == 'a.cs') if self.queue().next() else None
    ...
    def test_unknown_expired_effects_quarantine_queue(self):
        ...
        self.create()
        receipt = self.leased(lease_seconds=10)
        self.assertEqual(self.queue().reconcile(receipt, effects='unknown', execute=True, now=111)['status'], 'failed')
    ...
    def test_artifact_write_failure_keeps_lease_and_retry_is_explicit(self):
        ...
        with patch.object(q, 'write_immutable', side_effect=OSError('fixture disk failure')):
            with self.assertRaises((q.Blocked, OSError)):
                self.queue().complete(proposal, execute=True, now=101)
        self.assertEqual(self.queue().monitor()['states']['leased'], 1)
        self.assertEqual(self.queue().complete(proposal, execute=True, now=102)['status'], 'proposed')

    def test_published_result_commit_gap_recovers_without_overwrite(self):
        self.create()
        receipt = self.leased()
        proposal = self.proposal(receipt)
        with patch.object(q.Queue, '_record_result', side_effect=OSError('fixture crash'), create=True):
            with self.assertRaises(OSError):
                self.queue().complete(proposal, execute=True, now=101)
    ...
    def test_deferred_proposal_releases_paths_for_independent_work(self):
        ...
        self.assertEqual(self.queue().complete(proposal, execute=True, now=101)['status'], 'deferred')
        self.assertEqual(len(self.queue().next()), 1)
    ...
    def test_different_target_or_branch_binding_is_rejected(self):
        self.create()
        for settings in (dict(target=self.home), dict(branch='other')):
    ...
    def test_nonunique_or_unapproved_edit_is_not_a_proposal(self):
        self.create()
        receipt = self.leased()
        for path, old in [('b.cs', '=> 1'), ('../a.cs', '=> 1'), ('a.cs', 'not present'), ('a.cs', '')]:
            proposal = self.proposal(receipt)
    ...
    def test_revision_and_branch_mismatch_block_before_creation(self):
        for changes in ({'revision': 'b' * 40}, {'branch': 'other'}):
            with self.subTest(changes=changes), self.assertRaises(q.Blocked):
                q.slice_queue(self.target, self.export, self.state, 'feature/queue', execute=True,
                              identity_reader=lambda root: dict(self.identity(root), **changes))
        self.assertFalse(self.state.exists())
