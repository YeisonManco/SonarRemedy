...
class QueueCliTests(QueueFixture):
    def run_cli(self, *args, code=0, fixture=False):
        ...
        if fixture:
            wrapper = ('import sys; import debt_work; '
    ...
    def test_slice_default_is_compact_dry_run_without_git(self):
        result = self.run_cli('slice', '--target', self.target, '--export', self.export, '--branch', 'feature/queue')
    ...
    def test_fixture_cli_slice_claim_complete_and_document(self):
        self.assertEqual(self.run_cli('slice', '--target', self.target, '--export', self.export,
                                     '--branch', 'feature/queue', '--execute', fixture=True)['status'], 'created')
    ...
    def test_missing_git_identity_does_not_initialize_fixture_repo(self):
        self.assertEqual(self.run_cli('slice', '--target', self.target, '--export', self.export,
                                     '--branch', 'feature/queue', '--execute', code=2)['status'], 'blocked')
        self.assertFalse((self.target / '.git').exists())
        self.assertFalse(self.state.exists())

    def test_help_documents_actual_commands(self):
        result = subprocess.run([sys.executable, '-B', str(PACK / 'debt_work.py'), '--help'],
                                capture_output=True, text=True, cwd=PACK, timeout=10)
    ...
    def test_watch_cli_streams_only_bounded_readonly_progress(self):
        self.create()
        before = self.files()
        result = subprocess.run([sys.executable, '-B', str(PACK/'debt_work.py'), '--state', str(self.state),
                                 'watch', '--interval', '.01', '--duration', '.05'],
...
class RunnerTests(QueueFixture):
    def config(self):
        return dict(version=1, target=str(self.target), branch='feature/queue', policy='characterization',
                    characterization_reason='Explicit fixture-only whitespace refactor policy.', test_paths=[], expected_red={},
                    allowed_outputs=[], checks=[dict(name='build', kind='build', argv=[sys.executable, '-B', '-c', 'pass'],
                        executable_sha256=q.digest(Path(sys.executable).read_bytes()), cwd='.', timeout_seconds=10),
                        dict(name='tests', kind='trx', argv=[sys.executable, '-B', '-c', 'pass', '{run}'],
                        executable_sha256=q.digest(Path(sys.executable).read_bytes()), cwd='.', timeout_seconds=10, report='tests.trx')])

    def factory(self, context):
        source = context['sources'][0]
        return dict(version=1, job_id=context['job_id'], attempt_id=context['attempt_id'], lease=context['lease'],
                    context_fingerprint=context['context_fingerprint'], status='proposed', reason='', risks=[],
                    test_plan='Characterization fixture only; no model runs tests.', edits=[dict(path=source['path'],
                    before_sha256=source['sha256'], replacements=[dict(old='Value()', new='Value( )')])])

    def check(self, argv, cwd, **kwargs):
        if argv[-1] != 'pass':
            (Path(argv[-1]) / 'tests.trx').write_text(trx())
        return dict(status='exited', reason='', exit_code=0, stdout=b'')
    ...
    def test_native_execute_unavailable_without_state_or_auth_probe(self):
        ...
        with patch.object(q.Queue, '_open', side_effect=AssertionError('opened queue')):
            for provider in ('opencode', 'codex', 'claude', 'copilot'):
                self.assertEqual(r.run(self.queue(), provider=provider, execute=True)['status'], 'unavailable')
        self.assertEqual(before, self.files())
    ...
    def test_1205_findings_more_than_eight_files_integrate_across_batches_and_resume(self):
        ...
        first = r.run(self.queue(), execute=True, integrate=True, max_batches=1, proposal_factory=self.factory,
                      process_runner=self.check, control_root=self.home/'control')
        self.assertEqual(first['status'], 'batch_limit')
        second = r.run(self.queue(), execute=True, resume=True, integrate=True, proposal_factory=self.factory,
                       process_runner=self.check, control_root=self.home/'control')
        ...
        before = self.files()
        self.assertEqual(r.run(self.queue(), execute=True, resume=True, integrate=True,
                              process_runner=self.check, control_root=self.home/'control')['status'], 'quiescent')
        self.assertEqual(before, self.files())
    ...
    def test_all_deferred_quiescent_and_invalid_manual_result_does_not_loop(self):
        ...
        self.create()
        result = r.run(self.queue(), execute=True, proposal_factory=lambda context: {'wrong_id': 'x'})
    ...
    def test_facade_default_dry_run_across_four_hosts(self):
        ...
        before = self.files()
        for provider in ('opencode', 'codex', 'claude', 'copilot'):
            result = subprocess.run([pwsh, '-NoProfile', '-File', str(PACK/'debt-runner.ps1'),
                                     '-Provider', provider, '-State', str(self.state)],
    ...
    def test_partial_wrong_id_and_oversized_manual_results_fail_without_retry(self):
        for case in ('partial', 'wrong_id', 'oversized'):
            ...
            with self.subTest(case=case):
                self.assertEqual(r.run(self.queue(), execute=True, proposal_factory=invalid)['status'], 'quiescent')
                self.assertEqual(self.queue().monitor()['states']['failed'], 1)
    ...
    def test_integrated_failure_stops_sibling_writing(self):
        ...
        def fail(argv, cwd, **kwargs):
            ...
            if len(calls) == 3:
                result['exit_code'] = 1
            return result
        result = r.run(self.queue(), execute=True, integrate=True, proposal_factory=self.factory,
                       process_runner=fail, control_root=self.home/'control')
        self.assertEqual(result['status'], 'quarantined')
        self.assertEqual(len(calls), 3)
        changed = [name for name, before in original.items() if (self.target/name).read_bytes() != before]
        self.assertEqual(len(changed), 1, 'only the attempted integration may leave edits; its sibling is untouched')
