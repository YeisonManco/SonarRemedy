...
class TransportTests(unittest.TestCase):
    def opencode(self, text='{"status":"proposed"}'):
        return [dict(type='step_start', sessionID='s1', part=dict(type='step-start', messageID='m1')),
                dict(type='text', sessionID='s1', part=dict(type='text', messageID='m1', text=text, time={'end': 1})),
                dict(type='step_finish', sessionID='s1', part=dict(type='step-finish', messageID='m1', reason='stop'))]

    def codex(self):
        return [dict(type='thread.started', thread_id='s1'), dict(type='turn.started'),
                dict(type='item.completed', item=dict(id='i1', type='agent_message', text='{"status":"proposed"}')),
                dict(type='turn.completed', usage={'input_tokens': 1, 'output_tokens': 1})]
    ...
    def test_opencode_exact_text_and_terminal_contract(self):
        self.assertEqual(t.parse_events('opencode', lines(self.opencode()), session_id='s1', exit_code=0),
                         {'status': 'proposed'})

    def test_codex_exact_thread_and_turn_contract(self):
        self.assertEqual(t.parse_events('codex', lines(self.codex()), session_id='s1', exit_code=0),
                         {'status': 'proposed'})
    ...
    def test_wrong_session_nonzero_partial_missing_terminal_and_tool_use_rejected(self):
        variants = [self.opencode()[:-1], self.opencode() + [dict(type='tool_use', sessionID='s1')]]
        ...
        for session, exit_code in [('other', 0), ('s1', 1)]:
            with self.subTest(session=session, exit_code=exit_code), self.assertRaises(q.Blocked):
                t.parse_events('opencode', lines(self.opencode()), session_id=session, exit_code=exit_code)

    def test_codex_error_tool_duplicate_messages_and_missing_terminal_rejected(self):
        events = self.codex()
        variants = [events[:-1], events + [dict(type='error')],
                    events[:2] + [dict(type='item.completed', item=dict(type='command_execution'))] + events[2:],
    ...
    def test_no_first_json_search_or_sensitive_output_persistence(self):
        for text in ('noise {"status":"proposed"}', '```json\n{}\n```', '{"secret":"fixture-private"}'):
            with self.subTest(text=text), self.assertRaises(q.Blocked):
                t.parse_events('opencode', lines(self.opencode(text)), session_id='s1', exit_code=0,
    ...
    def test_real_child_exit_and_stderr_is_not_returned(self):
        result = t.run_process([sys.executable, '-B', '-c',
                                'import sys; print("ok"); print("fixture-private",file=sys.stderr); sys.exit(3)'], PACK)
    ...
    def test_real_child_timeout_and_output_bounds(self):
        for command, reason, limit in [('import time; time.sleep(30)', 'process_timeout', 1024),
                                       ('print("x"*100000)', 'process_output_limit', 64)]:
            start = time.monotonic()
            result = t.run_process([sys.executable, '-B', '-c', command], PACK, timeout=.3, output_limit=limit)
    ...
    def test_process_environment_drops_sonar_and_other_credentials(self):
        ...
        with patch.dict(os.environ, {'SONAR_TOKEN': 'fixture-private', 'PRIVATE_API_KEY': 'other-private'}):
    ...
    def test_process_environment_forwards_user_profile_folders_for_sdk_builds(self):
        ...
        code = ('import os; print(os.getenv("USERPROFILE")); print(os.getenv("APPDATA")); '
                'print(os.getenv("LOCALAPPDATA")); print(os.getenv("SONAR_TOKEN"))')
        with patch.dict(os.environ, {'USERPROFILE': r'C:\Users\fixture', 'APPDATA': r'C:\Users\fixture\AppData\Roaming',
                                     'LOCALAPPDATA': r'C:\Users\fixture\AppData\Local', 'SONAR_TOKEN': 'fixture-private'}):
            result = t.run_process([sys.executable, '-B', '-c', code], PACK)
        self.assertEqual(result['stdout'].splitlines(),
                         [br'C:\Users\fixture', br'C:\Users\fixture\AppData\Roaming',
                          br'C:\Users\fixture\AppData\Local', b'None'])
    ...
    def test_timeout_terminates_descendant_not_only_parent(self):
        ...
        result = t.run_process([sys.executable, '-B', '-c', code], PACK, timeout=.5)
    ...
    def test_stdin_empty_arguments_and_delayed_output_are_bounded(self):
        ...
        result = t.run_process([sys.executable, '-B', '-c', code, '', 'a b'], PACK,
                               timeout=2, input_data=b'bounded\ninput')
...
class ClaudeProfileTests(unittest.TestCase):
    def profile(self):
        return dict(version=1, name='claude-bare-api-key-v1', provider='claude', executable=sys.executable,
                    executable_sha256=t.executable_digest(sys.executable), cli_version='2.1.139',
                    model='claude-sonnet-4-6', destination='https://api.anthropic.com',
                    auth_env='ANTHROPIC_API_KEY', timeout_seconds=10, max_budget_usd=1)
    ...
    def test_invalid_profile_never_authorizes_arbitrary_tools_auth_or_destination(self):
        for values in ({'auth_env': 'SONAR_TOKEN'}, {'destination': 'https://other.example'},
                       {'name': 'general'}, {'tools': ['Bash']}, {'executable_sha256': '0'*64}):
            with self.subTest(values=values):
                self.assertEqual(t.capability('claude', dict(self.profile(), **values))['status'], 'unavailable')

    def test_fixed_factory_uses_only_restricted_native_argv_and_bounded_stdin(self):
        result = t.claude_invocation(self.profile(), {'job_id': 'j1'}, PACK, '00000000-0000-4000-8000-000000000001')
        self.assertIsNotNone(result)
        argv = result['argv']
        for flag in ('--bare', '-p', '--tools', '--strict-mcp-config', '--no-session-persistence', '--session-id'):
    ...
    def test_claude_exact_result_json_and_error_partial_multiple_tool_trace_rejection(self):
        envelope = dict(type='result', subtype='success', is_error=False, session_id='s1',
                        num_turns=1, stop_reason='end_turn', result='{"status":"proposed"}')
        try:
            actual = t.parse_events('claude', q.encoded(envelope), session_id='s1', exit_code=0)
        except q.Blocked:
            actual = None
        self.assertEqual(actual, {'status': 'proposed'})
        for changes in ({'is_error': True}, {'session_id': 'wrong'}, {'subtype': 'error_max_turns'},
                        {'stop_reason': 'max_tokens'}, {'num_turns': 2}, {'tool_calls': [{}]},
                        {'deferred_tool_use': {'name': 'Bash'}}, {'result': 'junk'}):
            with self.subTest(changes=changes), self.assertRaises(q.Blocked):
                t.parse_events('claude', q.encoded(dict(envelope, **changes)), session_id='s1', exit_code=0)
        with self.assertRaises(q.Blocked):
