...
class PackTests(unittest.TestCase):
    ...
    def test_token_in_code_paths_is_not_a_secret(self):
        ...
        for path in (
            "src/XM.Fronteras.Documentos.API/Middleware/ExtractTokenAttribute.cs",
    ...
    def test_one_running_job(self):
        self.export["issues"].append(
            {"id": "S2", "kind": "coverage", "path": "src/a.py", "line": 2}
        )
        state = d.plan(self.root, self.export, 2)
    ...
    def test_two_running_jobs_distinct_paths(self):
        ...
        self.export["issues"].append(
            {"id": "S2", "kind": "coverage", "path": "src/b.py", "line": 2}
        )
        state = d.plan(self.root, self.export, 2)
        d.transition(state, state["tasks"][0]["id"], "running")
        d.transition(state, state["tasks"][1]["id"], "running")
    ...
    def test_eighth_running_blocks_ninth(self):
        issues = [{"id": "S1", "kind": "smells", "path": "src/a.py", "line": 1}]
        for i in range(2, 10):
            (self.root / f"src/f{i}.py").write_text(f"x = {i}", encoding="utf-8")
            issues.append({"id": f"S{i}", "kind": "smells", "path": f"src/f{i}.py", "line": 1})
    ...
    def evidence(self):
        return {
            ...
            "sonar_timestamp": 990,
            "validated_timestamp": 995,
            "build_report": "reports/build.txt",
            "tests_report": "reports/tests.xml",
            "coverage_report": "reports/coverage.xml",
            "sonar_report": "reports/sonar.json",
        }
    ...
    def test_strict_thresholds_revision_and_freshness(self):
        ...
        for field, value in [
            ...
            with self.subTest(field=field, value=value):
                self.assertEqual(
                    d.validate(bad, "abc123", now=1000, max_age=60)["status"], "blocked"
                )
