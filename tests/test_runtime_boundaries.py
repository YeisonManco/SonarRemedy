...
class RuntimeBoundaries(unittest.TestCase):
    ...
    def test_real_process_redacts_and_returns_failure_code(self):
        ...
        rc = s.run_process([sys.executable, '-c', 'print("fixture-private"); raise SystemExit(3)'],
                           self.root, s.child_environment(), 10, log, s.redactor('fixture-private'))
    ...
    def test_real_process_timeout_is_bounded(self):
        ...
        with self.assertRaises(TimeoutError):
            s.run_process([sys.executable, '-c', 'import time; time.sleep(60)'], self.root,
                          s.child_environment(), .1, self.root / 'timeout.log', s.redactor('fixture'))
    ...
    def test_http_fixture_observes_bearer_only_at_configured_endpoint(self):
        ...
        class Opener:
            def open(inner, request, timeout):
                self.assertEqual(request.full_url, 'https://sonar.example.test/root/api/ce/task?id=one')
    ...
    def test_snapshot_binds_dirty_bytes_untracked_and_diff_without_exposing_content(self):
        ...
        def git(argv, **kwargs):
            command = argv[3]
            output = (str(self.root).encode() if argv[3:] == ['rev-parse', '--show-toplevel'] else
                      b'abc123' if command == 'rev-parse' else
                      b'opaque diff' if command == 'diff' else b'a.cs\0new.cs\0.debt-scan.lock\0')
    ...
    def test_real_cli_dry_run_and_no_skip_switch(self):
        ...
        config = dict(adapter='dotnet', repo=str(self.root), solution='App.sln',
                      project='fixture', branch='feature', url='https://sonar.example.test',
