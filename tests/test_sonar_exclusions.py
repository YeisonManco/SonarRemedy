...
class ExclusionScanTests(unittest.TestCase):
    ...
    def test_counts_exact_mixed_fixture(self):
        ...
        self.write(
            "src/App.csproj",
            "<Project><PropertyGroup><SonarQubeExclude>true</SonarQubeExclude>"
        ...
        self.assertEqual(
            result["notice"], "exclusions detected; metrics may be gamed — review before accepting"
        )
    ...
    def test_file_limit_exceeded_blocks(self):
        for i in range(30):
            self.write(f"src/f{i:02d}.cs", "// NOSONAR\n")
    ...
    def test_main_exit_codes_and_output(self):
        ...
        with (
            ...
            patch.object(
                sys, "argv", ["sonar_exclusions.py", str(self.repo), "--output", str(report)]
            ),
        ):
        ...
        with (
            ...
            patch.object(
                sys,
                "argv",
                ["sonar_exclusions.py", str(self.repo), "--output", str(self.repo / "report.json")],
            ),
        ):
            self.assertEqual(e.main(), 2)
