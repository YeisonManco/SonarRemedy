...
class FetchTests(unittest.TestCase):
    def setUp(self):
        ...
        self.raw = dict(adapter='api', repo=str(self.repo), url='https://sonar.example.test',
                        trusted_url='https://sonar.example.test', project='fixture',
                        branch='feature/test', timeout=30)
        self.config = f.Config(self.raw)
        self.calls = []
        self.responses = {
            'api/project_analyses/search': {'analyses': [{'key': 'analysis1',
                                                          'date': datetime.now(timezone.utc).isoformat(),
                                                          'revision': 'abc123'}]},
            'api/qualitygates/project_status': {'projectStatus': {'status': 'OK'}},
            'api/measures/component': {'component': {'key': 'fixture', 'measures': [
                {'metric': k, 'value': v} for k, v in dict(coverage='91', duplicated_lines_density='4',
                                                           code_smells='0', vulnerabilities='0',
                                                           security_hotspots='1',
                                                           security_hotspots_reviewed='100').items()]}},
            'api/issues/search': {'paging': {'pageIndex': 1, 'pageSize': 100, 'total': 3}, 'issues': [
                {'key': 'S1', 'project': 'fixture', 'component': 'fixture:src/A.cs', 'line': 2,
                 'type': 'CODE_SMELL', 'rule': 'csharpsquid:S2094'},
                {'key': 'V1', 'project': 'fixture', 'component': 'fixture:src/B.cs', 'line': 3,
                 'type': 'VULNERABILITY', 'rule': 'csharpsquid:S2077'},
                {'key': 'B1', 'project': 'fixture', 'component': 'fixture:src/C.cs', 'line': 4,
                 'type': 'BUG', 'rule': 'csharpsquid:S3776'}]},
            'api/hotspots/search': {'paging': {'pageIndex': 1, 'pageSize': 100, 'total': 1}, 'hotspots': [
                {'key': 'H1', 'project': 'fixture', 'component': 'fixture:src/D.cs', 'line': 5,
                 'status': 'TO_REVIEW', 'rule': 'csharpsquid:S2094'}]},
            'api/rules/search': {'total': 3, 'p': 1, 'ps': 50, 'rules': [
                {'key': 'csharpsquid:S2077', 'name': 'Formatting SQL queries is security-sensitive',
                 'description': 'D' * 250, 'remediation': {'func': 'CONSTANT_ISSUE', 'constantCost': '30min'}},
                {'key': 'csharpsquid:S2094', 'name': 'Classes should not be empty',
                 'description': 'E' * 250},
                {'key': 'csharpsquid:S3776', 'name': 'Cognitive Complexity of functions should not be too high',
                 'description': 'F' * 250}]},
        }
    ...
    def fetch(self, get=None, head='abc123'):
        ...
        with patch.dict(os.environ, {'SONAR_TOKEN': 'fixture-private-value'}), \
    ...
    def test_fetch_complete_normalizes_and_export_passes_plan(self):
        ...
        self.assertEqual(result['measures'], {'coverage': 91.0, 'duplication': 4.0, 'smells': 0.0,
                                              'vulnerabilities': 0.0, 'hotspots': 1.0})
        ...
        self.assertEqual(export, {'version': 1, 'revision': 'abc123', 'issues': [
            {'id': 'S1', 'kind': 'smells', 'path': 'src/A.cs', 'line': 2, 'rule': 'csharpsquid:S2094'},
            {'id': 'V1', 'kind': 'security', 'path': 'src/B.cs', 'line': 3, 'rule': 'csharpsquid:S2077'},
            {'id': 'B1', 'kind': 'smells', 'path': 'src/C.cs', 'line': 4, 'rule': 'csharpsquid:S3776'},
            {'id': 'H1', 'kind': 'security', 'path': 'src/D.cs', 'line': 5, 'rule': 'csharpsquid:S2094'}]})
    ...
    def test_reviewed_hotspots_and_unsupported_dispositions(self):
        ...
        for disposition in ({'status': 'OPEN'}, {'status': 'REVIEWED'},
    ...
    def test_analysis_lookup_http_403_degrades_to_head(self):
        ...
        def get(path, **params):
            if path == 'api/project_analyses/search':
                raise HTTPError('https://sonar.example.test/api/project_analyses/search', 403,
                                'Forbidden', [], None)
            return copy.deepcopy(self.responses[path])
    ...
    def test_analysis_lookup_empty_degrades_to_head(self):
        ...
        self.assertEqual(result['status'], 'collected')
        self.assertEqual(result['revision'], 'abc123')
        self.assertEqual(result['analysis_warning'], 'no branch analysis found; using local HEAD as revision')
        export = json.loads((self.root / 'runs' / 'export.json').read_text(encoding='utf-8'))
        self.assertEqual(export['revision'], 'abc123')

    def test_analysis_revision_invalid_degrades_to_head(self):
        self.responses['api/project_analyses/search'] = {'analyses': [{'key': 'analysis1',
                                                                       'revision': 'bad rev!'}]}
        with patch.object(f, 'git_dirty', return_value=False):
            result = self.fetch()
        self.assertEqual(result['status'], 'collected')
        self.assertEqual(result['revision'], 'abc123')
        self.assertEqual(result['analysis_warning'], 'analysis revision missing or invalid; using local HEAD')
        export = json.loads((self.root / 'runs' / 'export.json').read_text(encoding='utf-8'))
        self.assertEqual(export['revision'], 'abc123')
    ...
    def test_hotspots_lookup_http_403_degrades_to_issues_only(self):
        ...
        def get(path, **params):
            if path == 'api/hotspots/search':
                raise HTTPError('https://sonar.example.test/api/hotspots/search', 403,
                                'Forbidden', [], None)
            return copy.deepcopy(self.responses[path])
    ...
    def test_pagination_inconsistency_blocks(self):
        items = [{'key': 'S%03d' % i, 'project': 'fixture', 'component': 'fixture:src/A.cs',
                  'line': 1, 'type': 'CODE_SMELL'} for i in range(100)]

        def get(path, **params):
            if path == 'api/issues/search':
                return copy.deepcopy({'paging': {'pageIndex': params['p'], 'pageSize': 100,
    ...
    def test_foreign_project_component_blocks(self):
        self.responses['api/issues/search']['issues'][0] = {'key': 'S1', 'project': 'other',
                                                            'component': 'other:a.cs', 'line': 1,
                                                            'type': 'CODE_SMELL', 'rule': 'csharpsquid:S2094'}
        with self.assertRaises(f.Blocked):
            self.fetch()
    ...
    def test_rules_json_written_compact(self):
        ...
        self.assertEqual(set(rules), {'csharpsquid:S2094', 'csharpsquid:S2077', 'csharpsquid:S3776'})
        self.assertEqual(rules['csharpsquid:S2077']['name'], 'Formatting SQL queries is security-sensitive')
        ...
        self.assertEqual(calls[0][1]['rule_keys'], 'csharpsquid:S2077,csharpsquid:S2094,csharpsquid:S3776')
    ...
    def test_git_head_requires_root_and_valid_revision(self):
        def git(argv, **kwargs):
            command = argv[3]
            output = (str(self.repo).encode() if argv[3:] == ['rev-parse', '--show-toplevel'] else
                      b'abc123' if command == 'rev-parse' else b'')
    ...
    def test_severities_field_accepts_known_values_and_rejects_others(self):
        ...
        for value in ('blocker', 'BLOCKER,', ',CRITICAL', 'BLOCKER;CRITICAL', 'BLOCKER,BOGUS',
                      'BLOCKER,BLOCKER', '', 123, ['BLOCKER']):
