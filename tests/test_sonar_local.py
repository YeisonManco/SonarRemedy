...
class LocalTests(unittest.TestCase):
    def setUp(self):
        ...
        self.raw = dict(adapter='dotnet', repo=str(self.repo), solution='App.sln',
                        url='https://sonar.example.test', trusted_url='https://sonar.example.test',
                        project='fixture', branch='feature/test', timeout=30, poll_attempts=2)
    ...
    def runner(self, argv, cwd, env, timeout, log, redact):
        ...
        if argv[1] == 'test':
            ...
            (results / 'a.trx').write_text('<TestRun><ResultSummary outcome="Completed"><Counters total="2" executed="2" passed="2" failed="0" notExecuted="0"/></ResultSummary></TestRun>')
            (results / 'coverage.opencover.xml').write_text('<CoverageSession><Summary numSequencePoints="2" visitedSequencePoints="2"/></CoverageSession>')
        if argv[1:3] == ['sonarscanner', 'end']:
            report = self.repo / '.sonarqube/out/.sonar/report-task.txt'
            report.parent.mkdir(parents=True, exist_ok=True)
            report.write_text('projectKey=fixture\nceTaskId=task1\nserverUrl=' + self.gateway_url + '\nceTaskUrl=' + self.gateway_url + '/api/ce/task?id=task1\n')
        return 0
    ...
    def scan(self, runner=None):
        ...
        with patch.dict(os.environ, {'SONAR_TOKEN': 'fixture-private-value'}), \
                ...
                patch.object(s, 'collect', return_value={'analysis_id': 'analysis1', 'global_pass': False, 'issues': []}):
            return s.scan(self.config, self.root / 'runs', execute=True, runner=runner or self.runner)

    def test_dry_run_never_invokes_runner_or_network(self):
        with patch.dict(os.environ, {'SONAR_TOKEN': 'fixture-private-value'}), patch.object(s, 'Gateway', side_effect=AssertionError):
            result = s.scan(self.config, self.root / 'runs', runner=lambda *a: self.fail('executed'))
    ...
    def test_configuration_fails_closed(self):
        for changes in ({'url': 'http://sonar.example.test'}, {'trusted_url': 'https://other.test'},
                        {'url': 'https://user:pass@sonar.example.test'}, {'solution': '../App.sln'},
                        {'adapter': 'node'}, {'timeout': 0}, {'skip_tests': True}, {'token': 'bad'}):
    ...
    def test_successful_fixture_records_proof_not_global_approval(self):
        ...
        self.assertEqual([c[1] for c in self.calls], ['sonarscanner', 'build', 'test', 'sonarscanner'])
        run = Path(result['run'])
    ...
    def test_missing_and_zero_tests_block_upload(self):
        for zero in (False, True):
            def no_tests(argv, *args):
                rc = self.runner(argv, *args)
                if argv[1] == 'test':
                    f = Path(argv[argv.index('--results-directory') + 1]) / 'a.trx'
                    if zero:
                        f.write_text('<TestRun><ResultSummary><Counters total="0" executed="0" passed="0" failed="0" notExecuted="0"/></ResultSummary></TestRun>')
                    else:
                        f.unlink()
                return rc
    ...
    def test_redaction_covers_plain_urlencoded_and_basic(self):
        ...
        message = secret + ' ' + quote(secret, safe='') + ' ' + base64.b64encode((secret + ':').encode()).decode()
    ...
    def test_failed_trx_with_zero_exit_is_failed_not_blocked(self):
        def runner(argv, *args):
            ...
            if argv[1] == 'test':
                f = Path(argv[argv.index('--results-directory') + 1]) / 'a.trx'
                f.write_text(f.read_text().replace('passed="2" failed="0"', 'passed="1" failed="1"'))
            return rc
    ...
    def test_verification_requires_retained_test_and_coverage_hashes(self):
        ...
        for suffix in ('.trx', 'coverage.opencover.xml'):
            with self.subTest(suffix=suffix):
                ...
                run = Path(result['run'])
                result['reports'] = {k: v for k, v in result['reports'].items() if not k.endswith(suffix)}
    ...
    def test_accept_job_checks_scope_and_keeps_global_result_separate(self):
        ...
        before = {'snapshot': {'digest': state['revision'], 'files': {'App.sln': 'old'}},
                  'finished': 10, 'sonar': {'project': 'fixture', 'branch': 'feature',
                                          'issues': [{'id': 'S1'}]}}
        after = {'snapshot': {'files': {'App.sln': 'new'}}, 'started': 11,
                 'sonar': {'project': 'fixture', 'branch': 'feature',
                           'issues': [], 'global_pass': False}}
        ...
        with patch.object(s, 'verify_run', side_effect=[before, after]), self.assertRaises(s.Blocked):
