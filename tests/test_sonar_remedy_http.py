    ...
    def test_config_accepts_http_with_allow_http(self):
        with tempfile.TemporaryDirectory() as repo:
            cfg = sonar_fetch.Config({
                "adapter": "api", "repo": repo,
                "url": "http://sonar.local:9000", "trusted_url": "http://sonar.local:9000",
                "project": "p", "branch": "main", "allow_http": True,
            })
...
def _valid_rcfg(**overrides):
    cfg = {
        "version": 1,
        "sonar": {"url": "https://sonar.example.com", "project_key": "p", "token_env": "SONAR_TOKEN"},
        "repository": {"url": "https://github.com/o/r.git", "pat_env": "GIT_PAT",
                       "main_branch": "main", "propagation_branches": []},
        "worktrees": {"root": "C:/w"},
        "provider": "manual",
    }
...
class RecoverConfigHttpTests(unittest.TestCase):
    ...
    def test_prompt_sets_allow_http_for_http_url(self):
        ...
        cfg = rc.prompt(input_fn=lambda *a, **k: next(inputs),
                        secret_fn=lambda *a, **k: "sqa_TEST" if a and a[0].startswith("SonarQube token") else "ghp_TEST")
